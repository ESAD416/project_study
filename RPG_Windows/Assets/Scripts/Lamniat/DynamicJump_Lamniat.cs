    ...
    void OnTriggerEnter2D(Collider2D trigger)
    {
        ...
        if (trigger.gameObject.CompareTag("Map_Jump_Trigger")) 
        {
            ...
            if (CanJump)
            {
                ...
                // 触发跳跃逻辑
                m_Lamniat.StateController.SetCurrentBaseState(m_Lamniat.StateController.Jump);
                IsJumping = true;
                CanJump = false;
                //moveDuration = 0f;
                JumpCounter = 0;
    ...
    private void UpdateLamniatJumping() {
        ...
        else if (JumpCounter >= HeightDecreaseCount) 
        {
            ...
            if (TileUtils.HasTileAtPlayerPosition(currentTilemap, m_Lamniat.BodyCollider.bounds))
            {
                ...
                // 切換状态
                if(m_LamniatMovement.IsMoving) m_Lamniat.StateController.SetCurrentBaseState(m_Lamniat.StateController.Move);
                else m_Lamniat.StateController.SetCurrentBaseState(m_Lamniat.StateController.Idle);
